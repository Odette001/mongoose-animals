...
import { TestAnimal } from './testTypes';
...
const postAnimal = (app: Express, name: string, species: string, birthdate: Date, location: any): Promise<DBMessageResponseAnimals> => {
  return new Promise((resolve, reject) => {
    request(app)
      .post('/api/v1/animals')
      .send({ animal_name: name, species, birthdate, location })
      .expect(201, (err, res) => {
        if (err) return reject(err);
        resolve(res.body as DBMessageResponseAnimals);
      });
  });
};

const putAnimal = (app: Express, id: string, name: string, birthdate: Date, location: any): Promise<DBMessageResponseAnimals> => {
  return new Promise((resolve, reject) => {
    request(app)
      .put(`/api/v1/animals/${id}`)
      .send({ animal_name: name, birthdate, location })
      .expect(200, (err, res) => {
        if (err) return reject(err);
        resolve(res.body as DBMessageResponseAnimals);
      });
...
const getAnimals = async (app: Express) => {
  const res = await request(app).get('/api/v1/animals').expect(200);
  return res.body as TestAnimal[];
};

const getAnimalById = async (app: Express, id: string) => {
  const res = await request(app).get(`/api/v1/animals/${id}`).expect(200);
  return res.body as TestAnimal;
};

const getAnimalsByArea = async (app: Express, topRight: string, bottomLeft: string) => {
  const res = await request(app)
    .get(`/api/v1/animals/location?topRight=${topRight}&bottomLeft=${bottomLeft}`)
    .expect(200);
  return res.body as TestAnimal[];
};

const getaAnimalsBySpecies = async (app: Express, species_name: string) => {
  const res = await request(app)
    .get(`/api/v1/animals/species/${species_name}`)
    .expect(200);
  return res.body as TestAnimal[];
};

const deleteAnimal = async (app: Express, id: string) => {
  const res = await request(app).delete(`/api/v1/animals/${id}`).expect(200);
  return res.body as DBMessageResponseAnimals;
};

export { postAnimal, putAnimal, getAnimals, getAnimalById, getAnimalsByArea, getaAnimalsBySpecies, deleteAnimal };
      ...
      .expect(200, (err, response) => {
        if (err) {
          ...
          animals.forEach((animal) => {
            ...
            expect(
              ((animal.species as TestSpecies).category as unknown as TestCategory)
                .category_name,
            ).toBeDefined();
